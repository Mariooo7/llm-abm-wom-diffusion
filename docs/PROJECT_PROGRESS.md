...
- 正式实验前的研究与工程收敛
- 不改变研究语义，只做不影响因果解释的工程优化
- 以可复现、可统计检验、可追溯成本的方式启动正式实验
...
- 当前正式基线参数：A/C `p=0.003, q=0.20`；B/D `p=0.003, q=0.20`
- 独立冷启动种子比例：`initial_seed_ratio=0.04` (N=100 时固定 4 人)
- 当前 WOM 机制：离线语料环境分布抽样，强组(A/C) `high_arousal_ratio=0.6`，弱组(B/D) `high_arousal_ratio=0.3`
- 记忆机制：`memory_limit=5`
- 当前说明口径：本文件中更早的校准小节保留为历史演进过程，以本基线为准。
...
- 下一步方向（保持单旋钮原则）：
  - 暂不改 `q`，先在固定 `q` 下做 `p` 的小步校准（如 `0.002 -> 0.003`）并复用同一固定种子策略
...
- 证据摘要：
  ...
  - D 组：早期进度显示低采纳（step=3 时为 0），重试已中止
